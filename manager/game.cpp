...
void Game::LoadSetup() {
	...
	for (int ii = 0; ii <= (int)(Piece::BOMB); ++ii)
		usedUnits[ii] = 0;
	AddPiecesFromArrayToBoard(redSetup, Piece::RED, usedUnits);
	AddPiecesFromArrayToBoard(blueSetup, Piece::BLUE, usedUnits);
	...
	for (int y = 0; y < theBoard.Height(); ++y) {
		for (int x = 0; x < Game::theGame->theBoard.Width(); ++x) {
			...
			if (type != Piece::NOTHING && type != Piece::BOULDER) {
				...
				Game::theGame->theBoard.AddPiece(x, y, type, colour);
			}
		}
	}
}
